...
import { SafeWithdrawalResultsSection } from './sections/SafeWithdrawalResultsSection'
import type { SafeWithdrawalResponse } from '../../services/api'
import type { ChartDataPoint, CashflowFrequency } from './types'

export interface SafeWithdrawalResultsModalProps {
    isOpen: boolean
    onClose: () => void
    isLoading: boolean
    results: SafeWithdrawalResponse | null
...
const SafeWithdrawalResultsModalComponent: React.FC<SafeWithdrawalResultsModalProps> = ({
    isOpen,
    onClose,
    isLoading,
    results,
                ...
                <div className={`flex items-center justify-between px-4 sm:px-6 py-4 border-b ${isDark ? 'border-white/[0.1]' : 'border-gray-200'}`}>
                    <div>
                        <h1 className={`text-base font-semibold ${isDark ? 'text-white' : 'text-gray-900'}`}>
                            {isLoading ? 'Calculating Your Budget...' : 'Your Safe Withdrawal Budget'}
                        </h1>
                        <p className={`text-xs ${isDark ? 'text-gray-500' : 'text-gray-500'}`}>
                            {isLoading
                                ? 'Finding optimal spending at multiple confidence levels'
...
export const SafeWithdrawalResultsModal = memo(SafeWithdrawalResultsModalComponent)
SafeWithdrawalResultsModal.displayName = 'SafeWithdrawalResultsModal'

export default SafeWithdrawalResultsModal
